...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    private func startOverlayAndCapture() {
        ...
        // "Click to pop" flips the (full-screen) overlay to catch clicks; otherwise click-through.
        settings.$clickToPop
            .receive(on: RunLoop.main)
            .sink { [weak self] on in self?.window?.ignoresMouseEvents = !on }
            .store(in: &cancellables)
    ...
    private func setPetVisible(_ show: Bool) {
        if show {
            if petWindow == nil {
                let p = PetWindow(device: device, settings: settings)
    ...
    private func buildMenu() {
        ...
        let menu = NSMenu()
        let header = NSMenuItem(title: "Ghostty Black Holes", action: nil, keyEquivalent: "")
